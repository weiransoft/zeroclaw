...
#[tokio::test]
async fn test_minimal_orchestrator() {
    let tmp = TempDir::new().unwrap();
    let workspace_dir = tmp.path().join("workspace");
    tokio::fs::create_dir_all(&workspace_dir).await.unwrap();

    tracing::info!("=== Minimal Orchestrator Test ===");
...
Step 2: Use sessions_spawn to create sub-agents for specific subtasks
Step 3: Wait for sub-agents to complete and report results
Step 4: Synthesize and summarize all results
