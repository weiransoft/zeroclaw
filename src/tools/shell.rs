...
/// Safe PATH environment variable for Unix systems.
/// Only includes standard system directories to prevent PATH injection attacks.
const SAFE_PATH_UNIX: &str = "/usr/bin:/bin:/usr/sbin:/sbin";
/// Safe PATH environment variable for Windows systems.
const SAFE_PATH_WINDOWS: &str = "C:\\Windows\\System32;C:\\Windows";
...
impl Tool for ShellTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // 显式设置安全的 PATH，防止 PATH 注入攻击
        // 使用硬编码的系统路径白名单，避免从环境变量继承不安全的 PATH
        #[cfg(unix)]
        cmd.env("PATH", SAFE_PATH_UNIX);
        
        #[cfg(windows)]
        cmd.env("PATH", SAFE_PATH_WINDOWS);
    ...
    fn clone_box(&self) -> Box<dyn Tool> {
        Box::new(ShellTool::new(self.security.clone(), self.runtime.clone()))
    }
}
