...
use crate::config::{DelegateAgentConfig, ReliabilityConfig};
...
pub struct DelegateTool {
    ...
    depth: u32,
    /// Reliability configuration for provider creation
...
impl DelegateTool {
    ...
    /// Create a DelegateTool with reliability configuration.
...
impl Tool for DelegateTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // Create provider for this agent with retry and fallback support
        ...
        let provider: Box<dyn Provider> =
            match providers::create_resilient_provider(&agent_config.provider, api_key, &self.reliability) {
                Ok(p) => p,
                Err(e) => {
                    return Ok(ToolResult {
                        success: false,
