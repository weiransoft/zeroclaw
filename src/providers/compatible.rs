...
use crate::providers::traits::{ChatMessage, ChatResponse, Provider, ToolCall, TokenUsage};
...
impl OpenAiCompatibleProvider {
    pub fn new(name: &str, base_url: &str, api_key: Option<&str>, auth_style: AuthStyle) -> Self {
        Self {
            ...
            client: super::create_http_client(120),
        }
    }
...
fn map_response_message(message: ResponseMessage, usage: Option<ApiUsage>) -> ChatResponse {
...
impl Provider for OpenAiCompatibleProvider {
    async fn chat_with_system(
        ...
        self.chat_with_system_and_tools(system_prompt, message, model, temperature, None)
            .await
    }
    ...
    async fn chat_with_system_and_tools(
        ...
        tracing::debug!(
            ...
            tools_count = tools.map(|t| t.len()),
            "chat_with_system_and_tools called"
        );
        ...
        Ok(map_response_message(choice.message, chat_response.usage))
    }
    ...
    /// Override stream_chat to use real streaming API
    async fn stream_chat(
        ...
        temperature: f64,
        tools: Option<&[crate::tools::ToolSpec]>,
    ) -> anyhow::Result<tokio::sync::mpsc::Receiver<anyhow::Result<crate::providers::traits::StreamChunk>>> {
        ...
        tokio::spawn(async move {
            let api_key = match api_key {
                Some(k) => k,
                None => {
                    let _ = tx.send(Err(anyhow::anyhow!("{} API key not set", name))).await;
                    return;
                }
            };
            ...
            req = match auth_header {
                ...
                AuthStyle::XApiKey => req.header("x-api-key", &api_key),
                AuthStyle::Custom(header_name) => req.header(&header_name, &api_key),
            };
            ...
            if !response.status().is_success() {
                ...
                let _ = tx.send(Err(anyhow::anyhow!("{} API error ({}): {}", name, status, error))).await;
                return;
            }
                                ...
                                if let Some(delta) = &choice.delta {
                                    if let Some(content) = &delta.content {
                                        if !content.is_empty() {
                                            ...
                                            let _ = tx.send(Ok(crate::providers::traits::StreamChunk {
                                                text: content.clone(),
                                                is_final: false,
                                                tool_calls: vec![],
                                            })).await;
...
mod tests {
    ...
    fn response_with_tool_calls_maps_structured_data() {
        ...
        let mapped = map_response_message(choice.message, resp.usage);
    ...
    fn chat_completions_url_glm() {
        // GLM (BigModel) uses /api/paas/v4/chat/completions as full endpoint
        let p = make_provider("glm", "https://open.bigmodel.cn/api/paas/v4/chat/completions", None);
        assert_eq!(
            p.chat_completions_url(),
