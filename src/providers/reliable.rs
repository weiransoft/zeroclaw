...
impl Provider for ReliableProvider {
    ...
    async fn chat_with_system_and_tools(
        ...
        for current_model in &models {
            for (provider_name, provider) in &self.providers {
                ...
                let _provider_start = std::time::Instant::now();

                for attempt in 0..=self.max_retries {
                    let call_start = std::time::Instant::now();
                    match provider
                        .chat_with_system_and_tools(system_prompt, message, current_model, temperature, tools)
                        .await
                    {
                        Ok(resp) => {
    ...
    /// Stream chat with retry logic - delegates to underlying provider's stream_chat
    async fn stream_chat(
        ...
        temperature: f64,
        tools: Option<&[crate::tools::ToolSpec]>,
    ) -> anyhow::Result<tokio::sync::mpsc::Receiver<anyhow::Result<super::traits::StreamChunk>>> {
        // For streaming, we use the first provider directly without retry
        // Retry with streaming is complex and would require reconnection logic
        let (provider_name, provider) = self.providers.first()
            .ok_or_else(|| anyhow::anyhow!("No providers configured"))?;
        
        let current_model = self.model_fallbacks.get(model)
            .and_then(|v| v.first())
            .map(|s| s.as_str())
            .unwrap_or(model);
        ...
        provider.stream_chat(system_prompt, message, current_model, temperature, tools).await
    }
}
