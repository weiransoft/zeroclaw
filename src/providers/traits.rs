...
/// A stream chunk for streaming responses
#[derive(Debug, Clone)]
pub struct StreamChunk {
    ...
    pub is_final: bool,
    /// Tool calls requested by the LLM (only in final chunk)
    pub tool_calls: Vec<ToolCall>,
}
...
pub trait Provider: Send + Sync {
    ...
    /// Multi-turn conversation with tools. Default implementation extracts of last user
    ...
    /// Multi-turn conversation. Default implementation extracts of the last user
    /// message and delegates to `chat_with_system`.
    ...
    /// Stream chat response via channel.
    /// Returns a receiver that yields StreamChunk messages.
    /// Default implementation uses non-streaming version and sends one chunk.
    async fn stream_chat(
        ...
        let response = self.chat_with_system_and_tools(system_prompt, message, model, temperature, tools).await?;
