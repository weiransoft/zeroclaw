...
// TracedProvider 用于包装 Provider 以自动记录轨迹
pub use traced::TracedProvider;

use compatible::{AuthStyle, OpenAiCompatibleProvider};
use reliable::ReliableProvider;

/// Create a configured HTTP client for LLM API calls.
/// Uses consistent timeout and connection settings across all providers.
pub fn create_http_client(timeout_secs: u64) -> reqwest::Client {
...
pub fn create_routed_provider(
    ...
    if model_routes.is_empty() {
        // Return as Provider since StreamableProvider: Provider
        return Ok(create_resilient_provider(primary_name, api_key, reliability)?);
    }
