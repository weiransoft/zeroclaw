...
impl Provider for RouterProvider {
    ...
    async fn chat_with_system_and_tools(
        ...
        provider
            .chat_with_system_and_tools(system_prompt, message, &resolved_model, temperature, tools)
            .await
    }
    ...
    /// Stream chat - delegates to underlying provider's stream_chat
    async fn stream_chat(
        ...
        tracing::info!(
            provider = provider_name.as_str(),
            model = resolved_model.as_str(),
            "Router dispatching streaming request"
        );
        
        provider.stream_chat(system_prompt, message, &resolved_model, temperature, tools).await
    }
}
