...
pub(crate) async fn run_tool_call_loop(
    ...
    for _iteration in 0..max_tool_iterations {
        ...
        println!("[LLM Response] Full response text: {}", response_text);
        println!("[LLM Response] Tool calls: {:?}", response.tool_calls);
        tracing::info!("[LLM Response] Full response text: {}", response_text);
        tracing::info!("[LLM Response] Tool calls: {:?}", response.tool_calls);
        ...
        if !response.tool_calls.is_empty() {
            tracing::info!("[LLM Response] Using structured tool calls: {:?}", response.tool_calls);
        ...
        if tool_calls.is_empty() {
            ...
            if !tool_calls.is_empty() {
                tracing::info!("[LLM Response] Using fallback tool calls: {:?}", tool_calls);
            }
        }

        if tool_calls.is_empty() {
            tracing::info!("[LLM Response] No tool calls detected, returning text response");
        ...
        for call in &tool_calls {
            tracing::info!("[Tool Call] Executing tool: {}, arguments: {}", call.name, call.arguments);
            ...
            let result = if let Some(tool) = find_tool(tools_registry, &call.name) {
                match tool.execute(call.arguments.clone()).await {
                    Ok(r) => {
                        tracing::info!("[Tool Call] Tool {} completed with success: {}", call.name, r.success);
                    ...
                    Err(e) => {
                        tracing::error!("[Tool Call] Tool {} failed with error: {}", call.name, e);
                        observer.record_event(&ObserverEvent::ToolCall {
                            tool: call.name.clone(),
                            duration: start.elapsed(),
                            success: false,
...
pub async fn run(
    ...
    let mut system_prompt = if let Some(ref msg) = message {
        (*crate::channels::build_optimized_system_prompt(
            &config.workspace_dir,
            model_name,
            &tool_descs,
            &skills,
...
pub(crate) async fn process_message_with_swarm_context(
    ...
    let mut system_prompt = (*crate::channels::build_optimized_system_prompt(
        &config.workspace_dir,
        &model_name,
        &tool_descs,
        &skills,
