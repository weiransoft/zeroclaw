...
impl ConsensusManager {
    pub fn new(workspace_dir: &std::path::Path) -> Self {
        ...
        Self {
            chat_manager,
            store,
            boss_role: "Boss".to_string(),
        }
    }
    
    pub fn new_with_boss(workspace_dir: &std::path::Path, boss_role: String) -> Self {
        let chat_manager = Arc::new(SwarmChatManager::new(workspace_dir));
        let store = Arc::new(SwarmSqliteStore::new(workspace_dir));
        Self {
            chat_manager,
    ...
    pub fn get_consensus_state(&self, task_id: String) -> Result<TaskConsensus> {
        ...
        let mut boss_approval_received = false;

        for msg in &messages {
            participants.insert(msg.author.clone());
            
            // 检查是否需要 Boss 批准
            if msg.metadata.get("approval_needed").and_then(|v| v.as_bool()).unwrap_or(false) {
                approval_needed = true;
            }
            
            // 检查是否已收到 Boss 批准
            if msg.metadata.get("boss_approval").is_some() {
                boss_approval_received = true;
            }
        ...
        let status = if !disagreements.is_empty() && !approval_needed {
            ConsensusStatus::Disagreement
        } else if approval_needed && !boss_approval_received {
            ConsensusStatus::WaitingForBossApproval
        } else if consensus_request.is_none() {
    ...
    pub fn request_boss_approval(
        ...
        let content = if lang == "zh" {
            format!("需要 Boss 批准: {} (选项: {})", issue, options.join(", "))
        } else {
            format!("Boss approval needed: {} (options: {})", issue, options.join(", "))
        };
        ...
        let message_id = self.chat_manager.send_message(
            ...
            lang.to_string(),
            None,
            serde_json::json!({ "approval_needed": true, "issue": issue, "options": options, "requester": requester }),
        )?;
        
        tracing::info!(
            "[Consensus] Boss approval requested for task {} by {}: {}",
            task_id,
            requester,
            issue
        );
    ...
    pub fn provide_boss_approval(
        ...
        let content = if lang == "zh" {
            if approved {
                format!("Boss 批准: {}", decision)
            } else {
                format!("Boss 拒绝: {}", decision)
            }
        } else {
            if approved {
                format!("Boss approved: {}", decision)
            } else {
                format!("Boss rejected: {}", decision)
            }
        };
        ...
        let message_id = self.chat_manager.send_message(
            ...
            Some(task_id.clone()),
            approver.clone(),
            self.boss_role.clone(),
            ChatMessageType::Info,
            content,
            lang.to_string(),
            None,
            serde_json::json!({ "boss_approval": approved, "decision": decision, "approver": approver }),
        )?;
    ...
    pub fn needs_boss_approval(&self, task_id: String) -> Result<bool> {
        let state = self.get_consensus_state(task_id)?;
        Ok(state.status == ConsensusStatus::WaitingForBossApproval)
    }
}
