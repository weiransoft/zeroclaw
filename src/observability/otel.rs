    ...
    fn record_event(&self, event: &ObserverEvent) {
        ...
        match event {
            ...
            ObserverEvent::SwarmSpawn { run_id, agent_name, depth } => {
                let mut span = tracer.build(
                    opentelemetry::trace::SpanBuilder::from_name("swarm.spawn")
                        .with_kind(SpanKind::Internal)
                        .with_attributes(vec![
                            KeyValue::new("run_id", run_id.clone()),
                            KeyValue::new("agent", agent_name.clone()),
            ...
            ObserverEvent::SwarmComplete { run_id, agent_name, duration, success } => {
                ...
                let start_time = SystemTime::now()
                    ...
                    .unwrap_or(SystemTime::now());
                let mut span = tracer.build(
                    opentelemetry::trace::SpanBuilder::from_name("swarm.complete")
                        .with_kind(SpanKind::Internal)
                        .with_start_time(start_time)
                        .with_attributes(vec![
                            KeyValue::new("run_id", run_id.clone()),
            ...
            ObserverEvent::ProviderFallback { from_provider, to_provider, reason } => {
                let mut span = tracer.build(
                    opentelemetry::trace::SpanBuilder::from_name("provider.fallback")
                        .with_kind(SpanKind::Internal)
                        .with_attributes(vec![
            ...
            ObserverEvent::RateLimited { provider, retry_after_ms } => {
                let mut span = tracer.build(
                    opentelemetry::trace::SpanBuilder::from_name("provider.rate_limited")
                        .with_kind(SpanKind::Internal)
                        .with_attributes(vec![
                            KeyValue::new("provider", provider.clone()),
                            KeyValue::new("retry_after_ms", retry_after_ms.unwrap_or(0) as i64),
                        ]),
                );
                span.set_status(Status::error("rate_limited"));
                span.end();
            }
            ObserverEvent::ToolExecute { tool, action, duration, success, error } => {
                ...
                if let Some(e) = error {
                    span.set_attribute(KeyValue::new("error", e.clone()));
                }
                span.set_status(if *success { Status::Ok } else { Status::error("") });
                span.end();
            }
            ObserverEvent::DatabaseOperation { operation, table, duration, success } => {
                let secs = duration.as_secs_f64();
                let mut span = tracer.build(
                    opentelemetry::trace::SpanBuilder::from_name("db.operation")
                        .with_kind(SpanKind::Internal)
                        .with_attributes(vec![
            ...
            ObserverEvent::ConfigLoad { config_path, duration, success } => {
                let mut span = tracer.build(
                    opentelemetry::trace::SpanBuilder::from_name("config.load")
                        .with_kind(SpanKind::Internal)
                        .with_attributes(vec![
                            KeyValue::new("path", config_path.clone()),
                            KeyValue::new("success", *success),
            ...
            ObserverEvent::ProviderCreate { provider, duration, success } => {
                let mut span = tracer.build(
                    opentelemetry::trace::SpanBuilder::from_name("provider.create")
                        .with_kind(SpanKind::Internal)
                        .with_attributes(vec![
                            KeyValue::new("provider", provider.clone()),
                            KeyValue::new("success", *success),
            ...
            ObserverEvent::CryptoOperation { operation, duration, success } => {
                let mut span = tracer.build(
                    opentelemetry::trace::SpanBuilder::from_name("crypto.operation")
                        .with_kind(SpanKind::Internal)
                        .with_attributes(vec![
                            KeyValue::new("operation", operation.clone()),
                            KeyValue::new("success", *success),
                            KeyValue::new("duration_us", duration.as_micros() as i64),
                        ]),
                );
                span.set_status(if *success { Status::Ok } else { Status::error("") });
                span.end();
            ...
            ObserverEvent::QueueOperation { operation, queue_depth, duration } => {
                let mut span = tracer.build(
                    opentelemetry::trace::SpanBuilder::from_name("queue.operation")
                        .with_kind(SpanKind::Internal)
                        .with_attributes(vec![
                            KeyValue::new("operation", operation.clone()),
                            KeyValue::new("depth", *queue_depth as i64),
                            KeyValue::new("duration_us", duration.as_micros() as i64),
                        ]),
                );
                span.end();
