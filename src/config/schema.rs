...
pub struct ConfigVersion {
    ...
    /// 更新来源："auto_reload"（自动重载）、"api"（API 调用）、"cli"（命令行）
    pub updated_by: String,
}

impl ConfigVersion {
    /// 创建新的配置版本
    pub fn new(version: u64, updated_by: &str) -> Self {
        Self {
            version,
            updated_at: Utc::now(),
...
impl HotConfig {
    ...
    pub fn updated_at(&self) -> DateTime<Utc> {
        // 注意：这里简化实现，实际可以存储更新时间
        Utc::now()
    }
    
    /// 克隆内部配置（用于需要 Config 实例的场景）
    /// 
    /// # Returns
    /// 内部配置的克隆
...
/// Default provider name when not specified in config
...
/// Soul configuration for AI personality and expression style.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SoulConfig {
    /// Enable soul/personality system
    #[serde(default = "default_true")]
    pub enabled: bool,
    
    /// Soul preset name: "clara" (default), "zeroclaw", "technical_expert", etc.
    #[serde(default = "default_soul_preset")]
    pub preset: String,
    
    /// Custom soul configuration (overrides preset if provided)
    #[serde(default)]
    pub custom: Option<CustomSoulConfig>,
}
...
/// Custom soul configuration for fine-tuned personality
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CustomSoulConfig {
    /// Agent name
    ...
    /// Communication style: "direct", "friendly", "professional", "expressive", "technical", "adaptive"
    #[serde(default)]
    ...
    /// Emoji style: "none", "minimal", "natural", "frequent"
    #[serde(default)]
    ...
    /// Boundaries (things the agent will never do)
    #[serde(default)]
...
impl Default for CustomSoulConfig {
    fn default() -> Self {
        Self {
...
pub struct Config {
    ...
    /// Current agent name for sub-agent execution (used by group_chat tool)
    #[serde(skip)]
    ...
    /// Soul configuration for AI personality.
    #[serde(default)]
    pub soul: SoulConfig,
}
...
pub struct AgentConfig {
    ...
    /// Maximum tokens per conversation to prevent excessive usage.
    #[serde(default)]
...
pub struct SwarmConfig {
    ...
    #[serde(default)]
    pub orchestrator_prompt: Option<OrchestratorPromptConfig>,
}
...
pub struct WorkflowConfig {
    ...
    #[serde(default = "default_workflow_templates_dir")]
    ...
    /// Default Scrum workflow configuration
    #[serde(default)]
    pub scrum_config: ScrumWorkflowConfig,
    
    /// Software development team configuration
    #[serde(default)]
    pub dev_team_config: DevTeamConfig,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ScrumWorkflowConfig {
    /// Enable Scrum workflow
    #[serde(default)]
    pub enabled: bool,
...
pub struct DevTeamConfig {
    /// Enable software development team workflow
    #[serde(default)]
    pub enabled: bool,
    ...
    /// Development process
    #[serde(default = "default_dev_process")]
    pub process: String,
...
fn default_workflow_templates_dir() -> String {
    ".workspace/workflow_templates".into()
}

fn default_workflow_dir() -> String {
    ".workspace/workflows".into()
}
...
impl Default for WorkflowConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            templates_dir: default_workflow_templates_dir(),
            workflow_dir: default_workflow_dir(),
            default_roles: vec![
                "product_owner".into(),
                "scrum_master".into(),
                "architect".into(),
                "frontend_developer".into(),
                "backend_developer".into(),
                "qa_engineer".into(),
                "data_analyst".into(),
                "documentation_engineer".into(),
            ],
            default_phases: vec![
                "产品需求分析".into(),
                "架构设计".into(),
            ...
            scrum_config: ScrumWorkflowConfig::default(),
            dev_team_config: DevTeamConfig::default(),
        }
    }
}

impl Default for ScrumWorkflowConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            sprint_duration: default_sprint_duration(),
...
impl Default for DevTeamConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            team_members: Vec::new(),
            process: default_dev_process(),
            code_review_policy: "All code changes require peer review".into(),
            testing_strategy: "Test-driven development with unit tests and integration tests".into(),
        }
    }
}
...
pub struct DelegateAgentConfig {
    ...
    pub max_depth: u32,
    /// Soul preset for this sub-agent (independent from main agent)
    #[serde(default)]
    pub soul_preset: Option<String>,
}
...
impl Config {
    ...
    /// Update paired tokens in gateway config and persist to disk
    pub fn update_paired_tokens(&self, tokens: Vec<String>) -> Result<()> {
        tracing::debug!("Updating paired tokens (count: {}), config_path: {:?}", tokens.len(), self.config_path);
        
        let mut config_to_save = self.clone();
        config_to_save.gateway.paired_tokens = tokens;
        ...
        tracing::debug!("Successfully persisted paired tokens to config");
